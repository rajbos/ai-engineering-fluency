...
- **Family-pattern auto-resolution (no entry needed at all)**: For the five families above, `resolveMcpFamilyToolName` in `src/utils/toolUtils.ts` (mirrored in `.github/scripts/toolnames_utils.py` as `resolve_mcp_family_tool_name`) already recognizes *any* server-registration prefix as long as the id contains the family keyword (`github`, `playwright`, `context7`, `tavily`, `claude_browser`/`claude-in-chrome`) and ends in one of that family's known action names — regardless of how the specific MCP client/user spelled the server segment. This is what keeps a brand-new prefix spelling (a new IDE, a differently-named local server) from generating another "unknown tool" report or PR. **Before adding a new entry for a GitHub/Playwright/Context7/Tavily/Claude Browser tool, run `find_matches_for_new_tool` from `toolnames_utils.py` — if `family_equivalent` is non-empty, do not add it**, the tool already resolves and is already excluded from unknown-tool detection. Only add a literal `toolNames.json` entry when the tool is a genuinely new action name for that family (i.e. the upstream MCP server added a new tool) — and when you do, also add the action to the `actions` set in both `resolveMcpFamilyToolName` and `resolve_mcp_family_tool_name` so future prefix variants of that new action resolve automatically too.
