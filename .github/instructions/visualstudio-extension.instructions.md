...
- **Data layer**: The `VisualStudioDataAccess` class in `src/visualstudio.ts` already handles all session discovery and parsing for Visual Studio Copilot Chat binary session files (MessagePack format). The C# host reads session metadata and delegates token estimation to this layer by shelling out to the CLI, or by re-implementing only the discovery logic in C#.
...
Discovery is already implemented — see `src/visualstudio.ts` for `VisualStudioDataAccess.discoverSessions()`.
