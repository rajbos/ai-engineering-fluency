 ...
 * by path alone — only file content (the `entrypoint` field on early JSONL events, e.g.
 * "claude-desktop", "claude-cli", "claude-vscode") tells them apart.
	...
	try {
		...
		try {
			...
			for (const line of chunk.split('\n')) {
				...
				if (event?.entrypoint === 'claude-cli') { return 'Claude Code CLI'; }
