...
### Removed
- The "🔥 Mistral Cloud (Beta)" tab in the Diagnostic Report's Research group, along with the Mistral API key it stored. It was built on Mistral's beta Agents listing (`GET /v1/conversations`), which lists conversations *created through that API* — not Vibe cloud (web) sessions, which is what the tab claimed to show. It therefore returned an empty list for everyone, and fixing it was not possible: Vibe cloud sessions are served from a separate, session-cookie-authenticated surface on `chat.mistral.ai` that no API key can reach (a bearer request returns 401), and nothing in Mistral's published OpenAPI spec exposes them. Local Mistral Vibe sessions (`~/.vibe/logs/session/`) are unaffected and still counted everywhere. If you had connected a key, it is no longer read — you can delete it from your OS keychain under the extension's stored secrets
