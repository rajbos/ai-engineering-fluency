...
### Features
- **See whether your agent use is outrunning the foundations under it**, following GitHub's [Agentic Engineering System](https://github.com/resources/insights/agentic-engineering-system). Everything here stays on your machine; none of it is uploaded or shared. See [docs/features/AGENTIC-ENGINEERING-SIGNALS.md](../docs/features/AGENTIC-ENGINEERING-SIGNALS.md)
  - **AI Readiness → Adoption × foundations:** each scanned repository is placed by how much you use agents there against how well its controls support it. *Stretched* repositories (agent use ahead of the controls) list the controls to fix first. Controls the scan could not check are left out, not counted as missing.
  ...
  - Five new insights: a stretched repository, more agent use with more rework, agent sessions started without a stated goal needing more corrections, agent PRs without confirmed human review, and rising cloud-agent PR load with reverts or rework. The "add instructions" insight now cites your own with/without numbers when they qualify.
- **AI Readiness has its own tab in Usage Analysis**, also accessible from the Command Palette, instead of a section buried under the personal Fluency Score. It scans each repository's delivery and governance controls when opened, shows what blocks its next stage, and distinguishes missing controls from those it could not check. Opening the Fluency Score no longer scans repositories.
