...
### Performance
- Opening the Efficiency view no longer jumps to 96% and then sits there. The view builds its data from three separate passes over the session corpus (daily stats, usage analysis, per-session efficiency signals), and each pass used to parse the same files again from cold — so the bulk of the wait happened inside a step the progress bar drew as "the last 4%". The corpus is now parsed once up front to warm the shared session cache, and the three passes read from it; a refresh already running is awaited instead of raced with a second full walk, and an overlapping open and refresh share one pass
...
- Every loading screen's progress bar now only ever moves forward. The bar is one value with two bands — parsing fills the first 85%, the compute phase the rest — so the step from parsing into computing is an increase. Previously parsing ran to 100% and the compute step set it back to a fixed 96%. This applies to the Details panel and the desktop tray app as well, whose parsing progress now fills 0–85% rather than 0–100% before their compute step takes it to 96%
