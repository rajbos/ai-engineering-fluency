...
- The full-year daily-stats pass is now shared rather than duplicated. A refresh starts one detached in the background, and opening the Efficiency view or the Chart view could each start another over the same files; all callers now join a single walk. Efficiency builds are also serialized, so an open and a refresh can no longer interleave their writes to the shared stat caches and leave the older result behind
