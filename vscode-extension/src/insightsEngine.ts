...
export const INSIGHT_CATALOG: InsightDefinition[] = [
	...
	},
	{
		...
		buildBody: (ctx) => {
			const m = ctx.last30Days.modeUsage;
			const ask = m.ask ?? 0;
			const agentic = (m.agent ?? 0) + (m.cli ?? 0) + (m.cliApp ?? 0);
			const total = ask + (m.edit ?? 0) + agentic + (m.plan ?? 0) + (m.customAgent ?? 0);
		...
		appliesTo: (ctx) => {
			...
			const m = ctx.last30Days.modeUsage;
			const ask = m.ask ?? 0;
			// CLI interactions (terminal or Copilot desktop app) are autonomous
			// agent-mode-style usage — count them as agentic, not as "ask-only".
			const agentic = (m.agent ?? 0) + (m.cli ?? 0) + (m.cliApp ?? 0);
			const total = ask + (m.edit ?? 0) + agentic + (m.plan ?? 0) + (m.customAgent ?? 0);
